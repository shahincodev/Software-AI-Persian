...
**نسخه**: 0.3.0 (عامل هوشمند با فراخوانی ابزار ساختاریافته)  
...
| **فراخوانی ابزار ساختاریافته** | ✅ فعال | ۱۳ ابزار استاندارد با اعتبارسنجی schema و تلاش مجدد |
| **اعتبارسنجی خودکار** | ✅ فعال | هر فراخوانی ابزار قبل از اجرا اعتبارسنجی می‌شود |
| **تلاش مجدد هوشمند** | ✅ فعال | حداکثر ۲ بار تلاش مجدد با زمینه خطا برای خروج نامعتبر |
...
### معماری عامل‌محور (نسخه 0.3.0)

```
main.py (نقطه ورودی)
    ↓
SystemContext — ساخت زمینه بلادرنگ سیستم
    ↓
AIBrain.agent_chat() — AI با زمینه سیستم تصمیم می‌گیرد
    ├─ tool_call → اعتبارسنجی schema → تلاش مجدد در صورت خطا
    │   └─ action_factory → ActionController.execute_action()
...
۳. **Phase 3 — حلقه بینایی خودمختار**: یکپارچه‌سازی DesktopVision در حلقه اجرا، تأیید بصری.
