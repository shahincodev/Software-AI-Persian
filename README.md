...
**نسخه**: 0.2.0 (عامل هوشمند با دسترسی کامل به سیستم)  
...
- **فراخوانی ابزار ساختاریافته**: AI تصمیم می‌گیرد چه ابزاری فراخوانی شود (mkdir, ren, dir, ...)
...
| **فراخوانی ابزار** | ✅ فعال | execute_command, launch_app, list_directory, read_file, ... |
...
AIBrain.agent_chat() — AI با زمینه سیستم تصمیم می‌گیرد
    ├─ tool_call → ToolExecutor → اجرای دستورات سیستمی
...
۱. **تکمیل Phase 1**: اعتبارسنجی خط لوله سرتاسری NL → عامل → اجرا → بازخورد.
۲. **افزودن ابزارهای بیشتر**: read_file, write_file, copy, move, delete به عنوان ابزارهای استاندارد.
۳. **بهبود زمینه سیستم**: اضافه کردن اطلاعات برنامه‌های نصب شده، پنجره‌های فعال.
۴. **پشتیبانی چندزبانه**: بهبود پردازش فارسی در درخواست‌ها.
